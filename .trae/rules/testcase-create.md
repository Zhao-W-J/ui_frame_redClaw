...
## 目标要求
当用户请求生成功能测试用例时，直接创建CSV格式的测试用例文档，并将CSV文件存放在@testcase目录下。不需要先生成Markdown格式文件。

## CSV格式规范

### 文件命名规范
- 文件名格式：`{需求名称}-测试用例-{序号}.csv`
- 存放目录：`/testcase/`
- 示例：`/testcase/用户登录功能-测试用例-1.csv`

### CSV文件结构
CSV文件包含一个测试用例清单，使用UTF-8编码（带BOM标记），以逗号作为分隔符。

**重要说明**：为确保在WPS Office等软件中正确显示中文，CSV文件必须以UTF-8 BOM开头（即文件第一行开头添加�字符）。
...
2. **目录位置**：CSV文件必须存放在`/testcase/`目录下
...
- 测试步骤格式：每个步骤前加序号并换行，如"1.点击头像编辑按钮\n2.选择1MB的JPG格式图片\n3.点击确认上传"
- 预期结果明确且可验证
- CSV文件中包含换行和特殊字符的内容需要用双引号包围并转义处理
- **编码要求**：CSV文件必须以UTF-8 BOM开头，即第一行标题行前添加BOM标记（�字符）
...
5. **编码规范**：使用UTF-8编码（带BOM），确保在WPS Office等软件中正确显示中文
6. **BOM标记**：CSV文件第一行必须以BOM标记开头（�字符），解决WPS等软件的中文乱码问题
7. **字符转义**：CSV文件中包含逗号、双引号、换行符的内容需要用双引号包围并转义
8. **双引号转义**：CSV格式中，如果字段内容包含双引号，必须将内部的双引号转义为两个双引号（""），例如："点击""设置""按钮"
9. **多行内容格式**：包含换行符的字段（如测试步骤、预期结果）必须用双引号包围整个字段内容
10. **换行符处理规范**：
    - 测试步骤字段格式："1.操作步骤一\n2.操作步骤二\n3.操作步骤三"
    - 预期结果字段格式："1.预期结果一\n2.预期结果二\n3.预期结果三"
    - 所有包含换行符（\n）的字段内容必须用双引号包围
    - 字段内容中的双引号必须转义为两个双引号（""）
    - 确保CSV文件可以在Excel、WPS等软件中正确解析和显示
11. **列结构简化**：去除测试状态和备注列，保持9列结构
12. **覆盖率优先**：优先保证测试覆盖率，确保功能验证的完整性